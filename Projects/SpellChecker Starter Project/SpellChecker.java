...
public class SpellChecker {
	...
	public boolean isPalindrome(String word) {
		for (int i = 0; i < words.size(); i++) {
			if (words.get(i).equals(word)) {
				word = word.toLowerCase(); // parse inputs into a standard
											// format
	...
	public boolean wordHasRepeatedCharacters(String word) {
		for (int i = 0; i < word.length(); i++) {
			if ((i + 1 > word.length()) || (i + 2 > word.length())) {
				break;
			}
			String currpos = word.substring(i, i + 1);
			String nextpos = word.substring(i + 1, i + 2);
	...
	public ArrayList<String> anagrams(String word) {
		...
		for (int i = 0; i < words.size(); i++) {
			...
			if (wordThatIsBeingChecked.length() == size) {
				if ((isKnownWord(wordThatIsBeingChecked))) {
					if (isPalindrome(wordThatIsBeingChecked)
							&& paramWordIsAPalindrome) {
						for (int j = 0; j < size; j++) {
							if (wordThatIsBeingChecked.indexOf(word.substring(
									j, j + 1)) == -1) {
								break;
							} else if (j == (size - 1)) {
								if (!wordsThatMatch
										.contains(wordThatIsBeingChecked)) {
									if (!(wordHasRepeatedCharacters(wordThatIsBeingChecked))) {
										wordsThatMatch
												.add(wordThatIsBeingChecked);
										count++;
										System.out
												.println("(Line 106): The word "
														+ wordThatIsBeingChecked
														+ " will be added."
														+ " The number of words found so far is now "
														+ count + ".");
									}
								}
					...
					for (int k = 0; k < size; k++) {
						if (paramWordIsAPalindrome) {
							if (word.indexOf(wordThatIsBeingChecked.substring(
									k, k + 1)) == -1) {
								break; // stop the loop for checking in word,
										// and get a new word
							} else if (k == (size - 1)) {
								if (!wordsThatMatch
										.contains(wordThatIsBeingChecked)) {
				...
				continue; // the size of words.get(i) != word, so get the next
							// word.
			}
		}
		System.out.println();
		...
		// count++;
		// System.out.println("(Line 195): The word " + wordThatIsBeingChecked +
		// " will be added." + " The number of words found so far is now " +
		// count + ".");
		// }
		// break; //done, so get next word
		// }
		...
		// count++;
		// System.out.println("(Line 216): The word " + wordThatIsBeingChecked +
		// " will be added." + " The number of words found so far is now " +
		// count + ".");
		// }
		// break; //done, so get next word
		...
		// word = word.replaceFirst(word.substring(k, k + 1), "0");
		// count++;
		// System.out.println("(Line 234): The word " + wordThatIsBeingChecked +
		// " will be added." + " The number of words found so far is now " +
		// count + ".");
		// }
		// break; //done, so get next word
	...
	public ArrayList<String> wordsStartingWith(String pre) {
		...
		pre = pre.toUpperCase();
		for (int i = 0; i < words.size(); i++) {
			if (words.get(i).length() >= pre.length()
					&& words.get(i).substring(0, pre.length()).equals(pre)) {
	...
	public ArrayList<String> wordsContaining(String part) {
		...
		// else
		// {
		// if (words.get(i).length() >= part.length() &&
		// words.get(i).substring(0, part.length()).equals(part))
		// {
		// datWords.add(words.get(i));
		// }
		// while (i < x)
		// {
		// if (words.get(i).length() >= part.length() &&
		// words.get(i).substring(x, part.length()).equals(part))
		// {
		// datWords.add(words.get(i));
		// }
		// }
		...
		for (int i = 0; i < words.size(); i++) {
			...
			for (int j = 0; j < size; j++) {
				if (words.get(i).indexOf(part.substring(j, size)) == -1) {
					...
					wordsThatMatch.add(words.get(i));
					count++;
					System.out.println("The word " + words.get(i)
							+ " will be added."
							+ " The number of words found so far is now "
							+ count + ".");
				}
	...
	/**
	 * This method is done for you saves updated dictionary text file
	 */
	public void save() {
		reader.save(words);
	}
